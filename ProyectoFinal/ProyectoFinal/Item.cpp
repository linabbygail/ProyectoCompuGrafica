...
#include "Main.h"
	...
	//figures
	void CItem::esfera(GLfloat radio, int meridianos, int paralelos, GLuint text)
	{
	...
	void CItem::cilindro(float radio, float altura, int resolucion, GLuint text)
	{
	...
	void CItem::cono(float altura, float radio, int resolucion, GLuint text)
	{
	...
	//items
	void CItem::mueble(float altura, float largo, float profundidad, GLuint text, GLuint ladrillo)  //Funcion creacion prisma
	{
		...
		glBindTexture(GL_TEXTURE_2D, ladrillo);   // choose the texture to use.
		...
		glTexCoord2f(35.0f, 1.0f); glVertex3fv(vertice[7]);
		glTexCoord2f(35.0f, 0.0f); glVertex3fv(vertice[1]);
		glEnd();


		glBindTexture(GL_TEXTURE_2D, ladrillo);
		glBegin(GL_POLYGON);	//Right
								//glColor3f(0.0,0.0,1.0);
		glNormal3f(1.0f, 0.0f, 0.0f);
		glTexCoord2f(0.0f, 0.0f); glVertex3fv(vertice[0]);
		glTexCoord2f(35.0f, 0.0f); glVertex3fv(vertice[3]);
		glTexCoord2f(35.0f, 1.0f); glVertex3fv(vertice[5]);
		glTexCoord2f(0.0f, 1.0f); glVertex3fv(vertice[4]);
		glEnd();
	...
	void CItem::mueble2(float altura, float largo, float profundidad, GLuint text)  //Funcion creacion prisma
	{
	...
	void CItem::estufa(float altura, float largo, float profundidad, GLuint text, GLuint text2)  //Funcion creacion prisma
	{
	...
	void CItem::hoya(float radio, float altura, int resolucion, GLuint text, GLuint text2)
	{
		...
		for (int i = 0; i <resolucion; i++)
		{
			...
			v3[2] = radio*sin(angulo*(i + 1));
			//	
			v4[0] = (radio*cos(angulo*i)) / 1;
			v4[1] = altura / 1;
			v4[2] = (radio*sin(angulo*i)) / 1;

			v5[0] = (radio*cos(angulo*(i + 1))) / 1;
			v5[1] = altura / 1;
			v5[2] = (radio*sin(angulo*(i + 1))) / 1;
	...
	void CItem::maseta(float radio, float altura, int resolucion, GLuint text, GLuint text2)
	{
		...
		for (int i = 0; i <resolucion; i++)
		{
			...
			v4[0] = (radio*cos(angulo*i)) / 2;
			v4[1] = altura / 2;
			v4[2] = (radio*sin(angulo*i)) / 2;

			v5[0] = (radio*cos(angulo*(i + 1))) / 2;
			v5[1] = altura / 2;
			v5[2] = (radio*sin(angulo*(i + 1))) / 2;
	...
	void CItem::tecla(float altura, float largo, float profundidad, GLuint text)
	{
